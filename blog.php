...
<body>
    ...
    if (isset($_GET['cat'])) {
        $cat_id = $_GET['cat'];
        $cat_query = "SELECT * FROM tbl_desc_evento WHERE id_descevento = $cat_id";
    ...
    if (isset($_POST['search'])) {
        ...
        $all_posts_query = "SELECT * FROM tbl_eventos WHERE estado = 'publico'";
        $all_posts_query .= " and estiqueta LIKE '%$search%'";
        ...
        $all_posts_query = "SELECT * FROM tbl_eventos WHERE estado = 'publico'";
        if (isset($cat_name)) {
            $all_posts_query .= " and categoria = '$cat_name'";
        }
        $all_posts_run = mysqli_query($con, $all_posts_query);
        $all_posts = mysqli_num_rows($all_posts_run); 
        $total_pages = ceil($all_posts / $number_of_posts);    /* las dos lineas yo modifique*/
        $posts_start_from = ($page_id - 1) * $number_of_posts;
    }
    ?>
    ...
    <section>
        <div class="container">
            <div class="row">
                <div class="col-md-8">
                    <?php
                    $slider_query = "SELECT * FROM tbl_eventos WHERE estado = 'publico' ORDER BY id_eventos DESC LIMIT 5";  /*aqui tb modifique*/
                        ...
                        <div id="carousel-example-generic" class="carousel slide" data-ride="carousel">
                            ...
                            <div class="carousel-inner" role="listbox">
                                ...
                                while ($slider_row = mysqli_fetch_array($slider_run)) {
                                    $slider_id = $slider_row['id_eventos'];
                                    $slider_image = $slider_row['imagen_evento'];
                                    $slider_title = $slider_row['titulo'];
                                    $check = $check + 1;
                                    if ($check == 1) {
                ...
                if (isset($_POST['search'])) {
                    $search = $_POST['search-title'];
                    $query = "SELECT * FROM tbl_eventos WHERE estado = 'publico'";
                    $query .= " and estiqueta LIKE '%$search%'";
                    $query .= " ORDER BY id_eventos DESC LIMIT $posts_start_from, $number_of_posts";
                } else {
                    $query = "SELECT * FROM tbl_eventos WHERE estado = 'publico'";
                    if (isset($cat_name)) {
                        $query .= " and categoria = '$cat_name'";
                    }
                    $query .= " ORDER BY id_eventos DESC LIMIT $posts_start_from, $number_of_posts";
                }
