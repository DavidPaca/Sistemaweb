...
tbl_datos_personales_ninio.nombres, tbl_gustos_disgustos.id_gustos_disgustos, tbl_gustos_disgustos.detalle
FROM tbl_inter_ei_disgustos_ninio
INNER JOIN tbl_datos_personales_ninio
ON tbl_datos_personales_ninio.id_ninio = tbl_inter_ei_disgustos_ninio.id_ninio_dn
INNER JOIN tbl_gustos_disgustos
ON tbl_gustos_disgustos.id_gustos_disgustos = tbl_inter_ei_disgustos_ninio.id_disgustos_dn
WHERE tbl_inter_ei_disgustos_ninio.id_disgustos_dn = $id_alimento AND tbl_datos_personales_ninio.id_cdi = $_nom_cdi AND tbl_datos_personales_ninio.id_periodo_ninio = $_nom_periodo ORDER BY apellidos ASC"; 
...
$pdf -> Cell(30);  
$pdf -> Cell(120, 10, utf8_decode('Disgustos'), 0, 0, 'C');  
$pdf -> Ln(10);
...
while ($row = mysqli_fetch_array($resultado)){
    ...
    $nombres = $row['nombres'];
    $id_alimentos = $row['id_gustos_disgustos'];
    $detalle = $row['detalle'];
