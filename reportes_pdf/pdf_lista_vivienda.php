...
tbl_vivienda.id_vivienda, tbl_vivienda.detalle AS detalle_vivienda
FROM tbl_socio_economica
INNER JOIN tbl_datos_personales_ninio
ON tbl_datos_personales_ninio.id_ninio = tbl_socio_economica.id_ninio_se
INNER JOIN tbl_vivienda
ON tbl_vivienda.id_vivienda = tbl_socio_economica.vivienda_acupa_es
WHERE tbl_socio_economica.vivienda_acupa_es = $repor_id AND tbl_datos_personales_ninio.id_cdi = $_nom_cdi AND tbl_datos_personales_ninio.id_periodo_ninio = $_nom_periodo ORDER BY apellidos ASC"; 
...
$pdf -> Cell(30);  
$pdf -> Cell(120, 10, utf8_decode('Vivienda'), 0, 0, 'C');  
$pdf -> Ln(10);
...
while ($row = mysqli_fetch_array($resultado)){
    ...
    $nombres = $row['nombres'];
    $id_detalles = $row['id_vivienda'];
    $detalle_vivienda = $row['detalle_vivienda'];

  $pdf -> Cell(10, 6, $cont, 1, 0, 1 );
  $pdf -> Cell(30, 6, $numero_docide, 1, 0, 1 );
  $pdf -> Cell(75, 6, utf8_decode("$apellidos $nombres"), 1, 0, 1 );
  $pdf -> Cell(65, 6, utf8_decode($detalle_vivienda), 1, 1, 1 );
}
