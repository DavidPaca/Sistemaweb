...
tbl_datos_personales_ninio.nombres, tbl_parentesco.id_parentesco, tbl_parentesco.detalle
FROM tbl_inter_ei_relacion_ninio
INNER JOIN tbl_datos_personales_ninio
ON tbl_datos_personales_ninio.id_ninio = tbl_inter_ei_relacion_ninio.id_ninio_rn
INNER JOIN tbl_parentesco
ON tbl_parentesco.id_parentesco = tbl_inter_ei_relacion_ninio.id_parentesco_rn 
WHERE tbl_inter_ei_relacion_ninio.id_parentesco_rn = $id_alimento  AND tbl_datos_personales_ninio.id_cdi = $_nom_cdi AND tbl_datos_personales_ninio.id_periodo_ninio = $_nom_periodo ORDER BY apellidos ASC"; 
...
$pdf -> Cell(30);  
$pdf -> Cell(120, 10, utf8_decode('Persona que tiene más relación el niño'), 0, 0, 'C');  
$pdf -> Ln(10);
...
while ($row = mysqli_fetch_array($resultado)){
    ...
    $nombres = $row['nombres'];
    $id_alimentos = $row['id_parentesco'];
    $detalle = $row['detalle'];
