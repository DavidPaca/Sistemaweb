...
if (isset($_GET['del'])) {
    ...
    if ( $db_id_us_nivel == $tipo_user) {
        $del_query = "DELETE FROM tbl_medicamentos WHERE id_medicamentos = '$del_id'";
        if (mysqli_query($con, $del_query)) {
            $del_msg = "Medicamento ha sido eliminado";
        } else {
            $del_error = "Medicamento no ha sido eliminado";
        }
    }
}
...
if (isset($_POST['submit'])) {
    ...
    if (empty($cat_name)) {
        ...
        $check_query = "SELECT * FROM tbl_medicamentos WHERE detalle = '$cat_name'";
        $check_run = mysqli_query($con, $check_query);
        if (mysqli_num_rows($check_run) > 0) {
            $error = "Alimento ya existe";
        } else {
            $insert_query = "INSERT INTO tbl_medicamentos (detalle) VALUES ('$cat_name')";
            if (mysqli_query($con, $insert_query)) {
                $msg = "Medicamento ha sido agregado";
            } else {
                $error = "Medicamento no ha sido agregado";
            }
        }
    }
}
...
if (isset($_POST['update'])) {
    ...
    if (empty($cat_name)) {
        ...
        $check_run = mysqli_query($con, $check_query);
        if (mysqli_num_rows($check_run) > 0) {
            $up_error = "Medicamento ya existe";
        } else {
            $update_query = "UPDATE `tbl_medicamentos` SET `detalle` = '$cat_name' WHERE `tbl_medicamentos`.`id_medicamentos` = $edit_id";
            if (mysqli_query($con, $update_query)) {
                $up_msg = "Medicamento ha sido agregado";
            } else {
                $up_error = "Medicamento no ha sido agregado";
            }
        }
    }
}
                ...
                <div class="col-md-9">
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <form action="" method="post">
                                <div class="form-group">
                                    <label for="detalle">Nombre del medicamento:</label>
                                    <?php
                                    if (isset($msg)) {
                            ...
                            <?php
                            if (isset($_GET['edit'])) {
                                $edit_check_query = "SELECT * FROM tbl_medicamentos WHERE id_medicamentos = $edit_id";
                                ...
                                if (mysqli_num_rows($edit_check_run) > 0) {
                                    ...
                                    <form action="" method="post">
                                        <div class="form-group">
                                            <label for="tbl_medicamentos">Actualizar nombre del medicamento:</label>
                                            <?php
                                            if (isset($up_msg)) {
                                        ...
                                        <a href="promedicamentos_PROC.php?numcdi=<?php echo $num_cdi; ?>&numper=<?php echo $num_per; ?>">
                        ...
                        <div class="col-md-6"><br>
                            <?php
                            $get_query = "SELECT * FROM tbl_medicamentos ORDER BY detalle ASC";
                            ...
                            if (mysqli_num_rows($get_run) > 0) {
                                ...
                                <table class="table table-bordered table-striped table-hover" id="example" >
                                    ...
                                    <tbody>
                                        ...
                                        while ($get_row = mysqli_fetch_array($get_run)) {
                                            $cont++;
                                            $medica_id = $get_row['id_medicamentos'];
                                            $detallemedic_name = $get_row['detalle'];
                                            ?>
                                            <tr>
                                                <td><?php echo $cont; ?></td>
                                                <td><?php echo ($detallemedic_name); ?></td>
                                                <td><a href="promedicamentos_PROC.php?edit=<?php echo $medica_id; ?>&numcdi=<?php echo $num_cdi; ?>&numper=<?php echo $num_per; ?>"><i class="far fa-edit"></i></a></td>
                                                <td><a href="promedicamentos_PROC.php?del=<?php echo $medica_id; ?>&numcdi=<?php echo $num_cdi; ?>&numper=<?php echo $num_per; ?>" onclick="return confirm('¿Desea Borrar?');"><i class="fas fa-trash-alt"></i></a></td>
                                            </tr>
                                        <?php } ?>
                                    </tbody>
                                </table>
                                ...
                                echo "<center><h3>No se encontró lista de medicamentos</h3></center>";
                            }
                            ?>
                        </div>
                    </div>
