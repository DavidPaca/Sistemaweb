...
//echo $del_id;
    ...
    if ( $db_id_us_nivel == $tipo_user) {
        $del_query = "DELETE FROM tbl_gustos_disgustos WHERE id_gustos_disgustos = '$del_id'";
...
if (isset($_POST['submit'])) {
    ...
    if (empty($cat_name)) {
        ...
        $check_query = "SELECT * FROM tbl_gustos_disgustos WHERE detalle = '$cat_name'";
        $check_run = mysqli_query($con, $check_query);
        if (mysqli_num_rows($check_run) > 0) {
            $error = "Dato ya existe";
        } else {
            $insert_query = "INSERT INTO tbl_gustos_disgustos (detalle) VALUES ('$cat_name')";
...
if (isset($_POST['update'])) {
    ...
    if (empty($cat_name)) {
        ...
        if (mysqli_num_rows($check_run) > 0) {
            ...
            $update_query = "UPDATE `tbl_gustos_disgustos` SET `detalle` = '$cat_name' WHERE `tbl_gustos_disgustos`.`id_gustos_disgustos` = $edit_id";
    ...
    <div id="wrapper">
        ...
        <div class="container-fluid body-section">
            <div class="row">
                ...
                <div class="col-md-9">
                    <h1><i class="fa fa-file"></i> Personalidad </h1><hr>
                    <ol class="breadcrumb">
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            ...
                            <?php
                            if (isset($_GET['edit'])) {
                                $edit_check_query = "SELECT * FROM tbl_gustos_disgustos WHERE id_gustos_disgustos = $edit_id";
                                ...
                                if (mysqli_num_rows($edit_check_run) > 0) {
                                    ...
                                    <form action="" method="post">
                                        <div class="form-group">
                                            <label for="tbl_gustos_disgustos">Actualizar nombre:</label>
                                            <?php
                                            if (isset($up_msg)) {
                                        ...
                                        <a href="progustos_disgustos_PROC.php?numcdi=<?php echo $num_cdi; ?>&numper=<?php echo $num_per; ?>">
                        ...
                        <div class="col-md-6"><br>
                            <?php
                            $get_query = "SELECT * FROM tbl_gustos_disgustos ORDER BY detalle ASC";
                                ...
                                <table class="table table-bordered table-striped table-hover" id="example" >
                                    ...
                                    <tbody>
                                        ...
                                        while ($get_row = mysqli_fetch_array($get_run)) {
                                            $cont++;
                                            $vocabulario_id = $get_row['id_gustos_disgustos'];
                                            $detallevoca_name = $get_row['detalle'];
                                            ?>
                                            <tr>
                                                <td><?php echo $cont; ?></td>
                                                <td><?php echo ucfirst($detallevoca_name); ?></td>
                                                <td><a href="progustos_disgustos_PROC.php?edit=<?php echo $vocabulario_id; ?>&numcdi=<?php echo $num_cdi; ?>&numper=<?php echo $num_per; ?>"><i class="far fa-edit"></i></a></td>
                                                <td><a href="progustos_disgustos_PROC.php?del=<?php echo $vocabulario_id; ?>&numcdi=<?php echo $num_cdi; ?>&numper=<?php echo $num_per; ?>" onclick="return confirm('¿Desea Borrar?');"><i class="fas fa-trash-alt"></i></a></td>
                                            </tr>
                                        <?php } ?>
                                    </tbody>
                                </table>
