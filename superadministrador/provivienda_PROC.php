...
//echo $del_id;
    ...
    if ( $db_id_us_nivel == $tipo_user) {
        $del_query = "DELETE FROM tbl_vivienda WHERE id_vivienda = '$del_id'";
...
if (isset($_POST['submit'])) {
    ...
    if (empty($cat_name)) {
        ...
        $check_query = "SELECT * FROM tbl_vivienda WHERE detalle = '$cat_name'";
        $check_run = mysqli_query($con, $check_query);
        if (mysqli_num_rows($check_run) > 0) {
            $error = "Información existente";
        } else {
            $insert_query = "INSERT INTO tbl_vivienda (detalle) VALUES ('$cat_name')";
...
if (isset($_POST['update'])) {
    ...
    if (empty($cat_name)) {
        ...
        if (mysqli_num_rows($check_run) > 0) {
            ...
            $update_query = "UPDATE `tbl_vivienda` SET `detalle` = '$cat_name' WHERE `tbl_vivienda`.`id_vivienda` = $edit_id";
    ...
    <div id="wrapper">
        ...
        <div class="container-fluid body-section">
            <div class="row">
                ...
                <div class="col-md-9">
                    <h1><i class="fas fa-home"></i> Vivienda </h1><hr>
                    <ol class="breadcrumb">
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <form action="" method="post">
                                <div class="form-group">
                                    <label for="detalle">Nombre de la vivienda:</label>
                                    <?php
                                    if (isset($msg)) {
                                    ...
                                    ?>
                                    <input type="text" placeholder="Nombre de la vivienda" class="form-control" name="cat-name">
                                </div>
                            ...
                            <?php
                            if (isset($_GET['edit'])) {
                                $edit_check_query = "SELECT * FROM tbl_vivienda WHERE id_vivienda = $edit_id";
                                ...
                                if (mysqli_num_rows($edit_check_run) > 0) {
                                    ...
                                    <form action="" method="post">
                                        <div class="form-group">
                                            <label for="tbl_vivienda">Actualizar vivienda:</label>
                                            <?php
                                            if (isset($up_msg)) {
                                            ...
                                            ?>
                                            <input type="text" value="<?php echo $up_category; ?>" placeholder="Nombre de la vivienda" class="form-control" name="cat-name">
                                        </div>
                                        <input type="submit" value="Actualizar" name="update" class="btn btn-primary">
                                       
                                        <a href="provivienda_PROC.php?numcdi=<?php echo $num_cdi; ?>&numper=<?php echo $num_per; ?>">
                        ...
                        <div class="col-md-6"><br>
                            <?php
                            $get_query = "SELECT * FROM tbl_vivienda ORDER BY detalle ASC";
                                ...
                                <table class="table table-bordered table-striped table-hover" id="example" >
                                    ...
                                    <tbody>
                                        ...
                                        while ($get_row = mysqli_fetch_array($get_run)) {
                                            $cont++;
                                            $id_vivi = $get_row['id_vivienda'];
                                            $detallevivi_name = $get_row['detalle'];
                                            ?>
                                            <tr>
                                                <td><?php echo $cont; ?></td>
                                                <td><?php echo ucfirst($detallevivi_name); ?></td>
                                                <td><a href="provivienda_PROC.php?edit=<?php echo $id_vivi; ?>&numcdi=<?php echo $num_cdi; ?>&numper=<?php echo $num_per; ?>"><i class="far fa-edit"></i></a></td>
                                                <td><a href="provivienda_PROC.php?del=<?php echo $id_vivi; ?>&numcdi=<?php echo $num_cdi; ?>&numper=<?php echo $num_per; ?>" onclick="return confirm('¿Desea Borrar?');"><i class="fas fa-trash-alt"></i></a></td>
                                            </tr>
                                        <?php } ?>
                                    </tbody>
                                </table>
