...
//echo $del_id;
    ...
    if ( $db_id_us_nivel == $tipo_user) {
        $del_query = "DELETE FROM tbl_vocabulario WHERE id_vocabulario = '$del_id'";
        if (mysqli_query($con, $del_query)) {
            $del_msg = "Vocabulario ha sido eliminado";
        } else {
            $del_error = "Vocabulario no ha sido eliminado";
        }
    }
}
...
if (isset($_POST['submit'])) {
    ...
    if (empty($cat_name)) {
        ...
        $check_query = "SELECT * FROM tbl_vocabulario WHERE detalle = '$cat_name'";
        $check_run = mysqli_query($con, $check_query);
        if (mysqli_num_rows($check_run) > 0) {
            $error = "Vocabulario ya existe";
        } else {
            $insert_query = "INSERT INTO tbl_vocabulario (detalle) VALUES ('$cat_name')";
            if (mysqli_query($con, $insert_query)) {
                $msg = "Vocabulario ha sido agregado";
            } else {
                $error = "Vocabulario no ha sido agregado";
            }
        }
    }
}
...
if (isset($_POST['update'])) {
    ...
    if (empty($cat_name)) {
        ...
        if (mysqli_num_rows($check_run) > 0) {
            ...
            $update_query = "UPDATE `tbl_vocabulario` SET `detalle` = '$cat_name' WHERE `tbl_vocabulario`.`id_vocabulario` = $edit_id";
            if (mysqli_query($con, $update_query)) {
                $up_msg = "Vocabulario ha sido modificado";
            } else {
                $up_error = "Vocabulario no ha sido modificado";
            }
        }
    }
}
                ...
                <div class="col-md-9">
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            ...
                            <?php
                            if (isset($_GET['edit'])) {
                                $edit_check_query = "SELECT * FROM tbl_vocabulario WHERE id_vocabulario = $edit_id";
                                ...
                                if (mysqli_num_rows($edit_check_run) > 0) {
                                    ...
                                    <form action="" method="post">
                                        <div class="form-group">
                                            ...
                                            ?>
                                            <input type="text" value="<?php echo $up_category; ?>" placeholder="Nombre del tipo de vocabulario" class="form-control" name="cat-name">
                                        </div>
                                        <input type="submit" value="Actualizar" name="update" class="btn btn-primary">
                                       
                                        <a href="provocalbulario_PROC.php?numcdi=<?php echo $num_cdi; ?>&numper=<?php echo $num_per; ?>">
                        ...
                        <div class="col-md-6"><br>
                            <?php
                            $get_query = "SELECT * FROM tbl_vocabulario ORDER BY detalle ASC";
                                ...
                                <table class="table table-bordered table-striped table-hover" id="example" >
                                    ...
                                    <tbody>
                                        ...
                                        while ($get_row = mysqli_fetch_array($get_run)) {
                                            $cont++;
                                            $vocabulario_id = $get_row['id_vocabulario'];
                                            $detallevoca_name = $get_row['detalle'];
                                            ?>
                                            <tr>
                                                <td><?php echo $cont; ?></td>
                                                <td><?php echo ucfirst($detallevoca_name); ?></td>
                                                <td><a href="provocalbulario_PROC.php?edit=<?php echo $vocabulario_id; ?>&numcdi=<?php echo $num_cdi; ?>&numper=<?php echo $num_per; ?>"><i class="far fa-edit"></i></a></td>
                                                <td><a href="provocalbulario_PROC.php?del=<?php echo $vocabulario_id; ?>&numcdi=<?php echo $num_cdi; ?>&numper=<?php echo $num_per; ?>" onclick="return confirm('¿Desea Borrar?');"><i class="fas fa-trash-alt"></i></a></td>
                                            </tr>
                                        <?php } ?>
                                    </tbody>
                                </table>
