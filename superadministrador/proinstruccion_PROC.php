...
//echo $del_id;
    ...
    if ( $db_id_us_nivel == $tipo_user) {
        $del_query = "DELETE FROM tbl_instruccion WHERE id_instruccion = '$del_id'";
...
if (isset($_POST['submit'])) {
    ...
    if (empty($cat_name)) {
        ...
        $check_query = "SELECT * FROM tbl_instruccion WHERE detalle = '$cat_name'";
        $check_run = mysqli_query($con, $check_query);
        if (mysqli_num_rows($check_run) > 0) {
            $error = "Información existente";
        } else {
            $insert_query = "INSERT INTO tbl_instruccion (detalle) VALUES ('$cat_name')";
...
if (isset($_POST['update'])) {
    ...
    if (empty($cat_name)) {
        ...
        if (mysqli_num_rows($check_run) > 0) {
            ...
            $update_query = "UPDATE `tbl_instruccion` SET `detalle` = '$cat_name' WHERE `tbl_instruccion`.`id_instruccion` = $edit_id";
    ...
    <div id="wrapper">
        ...
        <div class="container-fluid body-section">
            <div class="row">
                ...
                <div class="col-md-9">
                    <h1><i class="fas fa-user-tie"></i> Instrucción </h1><hr>
                    <ol class="breadcrumb">
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <form action="" method="post">
                                <div class="form-group">
                                    <label for="detalle">Nombre de la instrucción:</label>
                                    <?php
                                    if (isset($msg)) {
                                    ...
                                    ?>
                                    <input type="text" placeholder="Nombre de la instrucción" class="form-control" name="cat-name">
                                </div>
                            ...
                            <?php
                            if (isset($_GET['edit'])) {
                                $edit_check_query = "SELECT * FROM tbl_instruccion WHERE id_instruccion = $edit_id";
                                ...
                                if (mysqli_num_rows($edit_check_run) > 0) {
                                    ...
                                    <form action="" method="post">
                                        <div class="form-group">
                                            <label for="tbl_instruccion">Actualizar vivienda:</label>
                                            <?php
                                            if (isset($up_msg)) {
                                            ...
                                            ?>
                                            <input type="text" value="<?php echo $up_category; ?>" placeholder="Nombre de la instrucción" class="form-control" name="cat-name">
                                        </div>
                                        <input type="submit" value="Actualizar" name="update" class="btn btn-primary">
                                       
                                        <a href="proinstruccion_PROC.php?numcdi=<?php echo $num_cdi; ?>&numper=<?php echo $num_per; ?>">
                        ...
                        <div class="col-md-6"><br>
                            <?php
                            $get_query = "SELECT * FROM tbl_instruccion ORDER BY detalle ASC";
                            ...
                            if (mysqli_num_rows($get_run) > 0) {
                                ...
                                <table class="table table-bordered table-striped table-hover" id="example" >
                                    ...
                                    <thead>
                                        <tr>
                                            <th>Número</th>
                                            <th>Nombre de la instrucción</th>
                                            <th>Editar</th>
                                            <th>Eliminar</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        ...
                                        while ($get_row = mysqli_fetch_array($get_run)) {
                                            $cont++;
                                            $id_vivi = $get_row['id_instruccion'];
                                            $detallevivi_name = $get_row['detalle'];
                                            ?>
                                            <tr>
                                                <td><?php echo $cont; ?></td>
                                                <td><?php echo ucfirst($detallevivi_name); ?></td>
                                                <td><a href="proinstruccion_PROC.php?edit=<?php echo $id_vivi; ?>&numcdi=<?php echo $num_cdi; ?>&numper=<?php echo $num_per; ?>"><i class="far fa-edit"></i></a></td>
                                                <td><a href="proinstruccion_PROC.php?del=<?php echo $id_vivi; ?>&numcdi=<?php echo $num_cdi; ?>&numper=<?php echo $num_per; ?>" onclick="return confirm('¿Desea Borrar?');"><i class="fas fa-trash-alt"></i></a></td>
                                            </tr>
                                        <?php } ?>
                                    </tbody>
                                </table>
