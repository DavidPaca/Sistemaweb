...
if (isset($_GET['del'])) {
    ...
    if ( $db_id_us_nivel == $tipo_user) {
        $del_query = "DELETE FROM tbl_parentesco WHERE id_parentesco = '$del_id'";
        if (mysqli_query($con, $del_query)) {
            $del_msg = "Parentesco ha sido eliminado";
        } else {
            $del_error = "Parentesco no ha sido eliminado";
        }
    }
}
...
if (isset($_POST['submit'])) {
    ...
    if (empty($cat_name)) {
        ...
        $check_query = "SELECT * FROM tbl_parentesco WHERE id_parentesco = '$cat_name'";
        $check_run = mysqli_query($con, $check_query);
        if (mysqli_num_rows($check_run) > 0) {
            $error = "Parentesco ya existe";
        } else {
            $insert_query = "INSERT INTO tbl_parentesco (detalle) VALUES ('$cat_name')";
            if (mysqli_query($con, $insert_query)) {
                $msg = "Parentesco ha sido agregado";
            } else {
                $error = "Parentesco no ha sido agregado";
            }
        }
    }
}
...
if (isset($_POST['update'])) {
    ...
    if (empty($cat_name)) {
        ...
        $check_query = "SELECT * FROM tbl_parentesco WHERE detalle = '$cat_name'";
        $check_run = mysqli_query($con, $check_query);
        if (mysqli_num_rows($check_run) > 0) {
            $up_error = "Parentesco ya existe";
        } else {
            $update_query = "UPDATE `tbl_parentesco` SET `detalle` = '$cat_name' WHERE `tbl_parentesco`.`id_parentesco` = $edit_id";
            if (mysqli_query($con, $update_query)) {
                $up_msg = "Parentesco ha sido agregado";
            } else {
                $up_error = "Parentesco no ha sido agregado";
            }
        }
    }
}
    ...
    <div id="wrapper">
        ...
        <div class="container-fluid body-section">
            <div class="row">
                ...
                <div class="col-md-9">
                    <h1><i class="fas fa-user-tie"></i> Parentesco </h1><hr>
                    <ol class="breadcrumb">
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <form action="" method="post">
                                <div class="form-group">
                                    <label for="detalle">Nombre del Parentesco:</label>
                                    <?php
                                    if (isset($msg)) {
                            ...
                            <?php
                            if (isset($_GET['edit'])) {
                                $edit_check_query = "SELECT * FROM tbl_parentesco WHERE id_parentesco = $edit_id";
                                ...
                                if (mysqli_num_rows($edit_check_run) > 0) {
                                    ...
                                    <form action="" method="post">
                                        <div class="form-group">
                                            <label for="tbl_parentesco">Actualizar parentesco:</label>
                                            <?php
                                            if (isset($up_msg)) {
                                            ...
                                            ?>
                                            <input type="text" value="<?php echo $up_category; ?>" placeholder="Nombre de parentesco" class="form-control" name="cat-name">
                                        </div>
                                        <input type="submit" value="Actualizar" name="update" class="btn btn-primary">
                                       
                                        <a href="proparentesco_PROC.php?numcdi=<?php echo $num_cdi; ?>&numper=<?php echo $num_per; ?>">
                        ...
                        <div class="col-md-6"><br>
                            <?php
                            $get_query = "SELECT * FROM tbl_parentesco ORDER BY detalle ASC";
                                ...
                                <table class="table table-bordered table-striped table-hover" id="example" >
                                    ...
                                    <tbody>
                                        ...
                                        while ($get_row = mysqli_fetch_array($get_run)) {
                                            $cont++;
                                            $medica_id = $get_row['id_parentesco'];
                                            $detallemedic_name = $get_row['detalle'];
                                            ?>
                                            <tr>
                                                <td><?php echo $cont; ?></td>
                                                <td><?php echo ($detallemedic_name); ?></td>
                                                <td><a href="proparentesco_PROC.php?edit=<?php echo $medica_id; ?>&numcdi=<?php echo $num_cdi; ?>&numper=<?php echo $num_per; ?>"><i class="far fa-edit"></i></a></td>
                                                <td><a href="proparentesco_PROC.php?del=<?php echo $medica_id; ?>&numcdi=<?php echo $num_cdi; ?>&numper=<?php echo $num_per; ?>" onclick="return confirm('¿Desea Borrar?');  "><i class="fas fa-trash-alt"></i></a></td>
                                            </tr>
                                        <?php } ?>
                                    </tbody>
                                </table>
